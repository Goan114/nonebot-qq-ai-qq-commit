...
- 指定群被动监听，或使用 `/反馈` 主动提交。AI 识别游戏问题、提取设备与浏览器、保留完整文本和 OneBot 消息段、概括与归类。
...
- 提交标题与代码 diff 一起交给 AI，通俗摘要可发送到指定群。所有仓库共享问题库，直接代码证据、较高置信度和完整 diff 才自动确认修复；其它建议进入人工审核。
...
每个分支有独立的轮询间隔、断点和推送群。消息会标明 `owner/game@develop` 等来源；同一 SHA 从 develop 合入 main 后，两个分支各自推送一次，重复轮询/Webhook 不会重复入队。AI 摘要及修复判断按仓库和 SHA 复用，同一问题不会因分支合并再次 @ 通知。首次建立某分支基线不补发历史提交。
...
从 0.1.x 升级会自动将数据库迁移至版本 2，保留原记录。升级前请备份数据库，升级后不要用旧程序直接打开已迁移的数据。
...
匹配每 30 条问题/FAQ 一批，修复分析每 20 个未解决问题一批，遍历全部候选，避免仅高优先级问题被分析。问题很多时会增加费用和延迟。被动全群监听默认开启；若群闲聊很多，可设置 `FEEDBACK_LISTEN_ALL=false`，只分析关键词命中与 `/反馈`，降低调用量。`FEEDBACK_KEYWORDS` 可配置 JSON 字符串数组。每个 QQ 默认 30 秒提交冷却，可配置为 0。
...
- 群反馈收集只使用单条消息，不拼接前后文。缺字段时需在一条新消息补齐。关联截图在视觉功能启用后参与分析，音频不参与 AI 分析。
